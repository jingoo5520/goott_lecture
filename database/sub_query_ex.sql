...
--1-1) smith는 빼고 출력
select ename, job from emp
where (job = (select job from emp where ename = upper('smith'))) and ename != upper('smith');

--2. 부서별로 가장 급여를 많이 받는 사원의 정보를 출력
select job, sal, ename from emp where sal in (select max(sal) from emp
group by job);

--3. 직급이 salesman인 사원이 받는 급여 중 최소 급여 보다 많이 받는 사원의 이름과 급여를 출력하되, 
--부서번호가 20번인 사원은 제외하여 출력
select ename, sal, deptno from emp where (sal > any(select sal from emp where job = upper('salesman'))) and deptno != 20;
